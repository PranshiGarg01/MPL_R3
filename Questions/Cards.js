$(document).ready(function () {
  ...
  $(".card-container").on("click", function () {
    ...
    if (category === "Games" && index === 5) {
      ...
      isWildCard = true;
    } else if (category === "Movies" && index === 4) {
      // New: Check for the Movies Wild Card
      isWildCard = true;
  ...
  $("#gamerevealButton").on("click", function () {
    const questionText =
      "This fighting game series features combatants as silhouetted figures with realistic, physics-based martial arts animations, where players must defeat a series of demon bosses, including the assassin Lynx, the pirate Wasp, and the tyrannical Shogun, to restore their own physical form.";
  ...
  $("#mathsRevealButton").on("click", function () {
    const questionText =
      "What is the name of a mathematical knot that cannot be unknotted without cutting it?";
    $("#wildcardPopup").css("display", "none");
    $("#revealedQuestionText").text(questionText);
    $("#questionPopup").css("display", "flex");
  ...
  $("#sportsRevealButton").on("click", function () {
    const audio = "../assets/sports_500.mp3";
    $("#wildcardPopup").css("display", "none");
